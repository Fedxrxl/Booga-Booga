...
local function getEntityIDFromPlayer(player)
    -- Try Character name first
    local entity = workspace:FindFirstChild(player.Name, true)
    if entity and entity:GetAttribute("EntityID") then
        return entity:GetAttribute("EntityID"), entity
    end

    -- Fallback: search all descendants for a matching model
...
local function swingPlayers()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= Player then
            local id, model = getEntityIDFromPlayer(plr)
            ...
            if id and model then
                ...
                if part and (root.Position - part.Position).Magnitude <= RANGE then
                    hits[#hits + 1] = {
                        entityID = id,
