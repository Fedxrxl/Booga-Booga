...
for _, itemName in ipairs(ForcedItems) do
	if not table.find(ItemList, itemName) then
		table.insert(ItemList, itemName)
	end
end
...
RunService.Heartbeat:Connect(function()
	...
	-- NORMAL ITEMS
	-- NORMAL ITEMS (FULL WORKSPACE SCAN)
	for _, item in pairs(workspace:GetDescendants()) do
		if pickupCount >= MAX_PICKUP_COUNT then break end

		-- only accept valid item models/parts
		if not item:GetAttribute("EntityID") then continue end

		-- must be in workspace (not replicatedstorage etc)
		if not item:IsDescendantOf(workspace) then continue end

		-- ignore anything inside chests / deployables if you want
		if item:IsDescendantOf(workspace:FindFirstChild("Deployables") or {}) then
			continue
		end
		...
		local distance = (charPos - pos).Magnitude
